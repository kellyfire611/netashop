...
<form name="frmUpdate" method="post" action="{{ route('backend.shop_products.update', ['id' => $updatingModel->id]) }}" enctype="multipart/form-data">
  @csrf
  @method('PUT')
  <div>
    <label for="product_code" class="form-label">Mã sản phẩm</label>
    <input type="text" name="product_code" class="form-control" id="product_code"
      placeholder="Mời nhập mã sản phẩm" value="{{ old('product_code', $updatingModel->product_code) }}">
  </div>

  <div>
    <label for="product_name" class="form-label">Tên sản phẩm</label>
    <input type="text" name="product_name" class="form-control" id="product_name"
      placeholder="Mời nhập tên sản phẩm" value="{{ old('product_name', $updatingModel->product_name) }}">
  </div>

  <div>
    <label for="short_description" class="form-label">Nội dung mô tả ngắn</label>
    <textarea type="text" name="short_description" class="form-control" id="short_description">{{ old('short_description', $updatingModel->short_description) }}</textarea>
  </div>
  ...
  <div>
    ...
    <input type="number" name="standard_cost" class="form-control" id="standard_cost"
      placeholder="Mời nhập giá nhập" value="{{ old('standard_cost', $updatingModel->standard_cost) }}">
  </div>
  ...
  <div>
    ...
    <input type="number" name="list_price" class="form-control" id="list_price"
      placeholder="Mời nhập giá niêm yết" value="{{ old('list_price', $updatingModel->list_price) }}">
  </div>
  ...
  <div>
    ...
    <input type="number" name="quantity_per_unit" class="form-control" id="quantity_per_unit"
      placeholder="Mời nhập giá niêm yết" value="{{ old('quantity_per_unit', $updatingModel->quantity_per_unit) }}">
  </div>

  <div class="mt-2">
    <div class="form-check form-switch">
      <input class="form-check-input" type="checkbox" role="switch" id="discontinued" name="discontinued" value="1" {{ old('discontinued', $updatingModel->discontinued) == "1" ? 'checked' : '' }}>
      <label class="form-check-label" for="discontinued">Ngưng sử dụng?</label>
    </div>
  </div>
  <div class="mt-2">
    <div class="form-check form-switch">
      <input class="form-check-input" type="checkbox" role="switch" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $updatingModel->is_featured) == "1" ? 'checked' : '' }}>
      <label class="form-check-label" for="is_featured">Là sản phẩm nổi bật?</label>
    </div>
  </div>
  <div class="mt-2">
    <div class="form-check form-switch">
      <input class="form-check-input" type="checkbox" role="switch" id="is_new" name="is_new" value="1" {{ old('is_new', $updatingModel->is_new) == "1" ? 'checked' : '' }}>
  ...
  <div class="mt-2">
    ...
    <select name="category_id" id="category_id" class="form-control">
      ...
      @foreach($dsCategories as $cat)
        @if(old('category_id', $updatingModel->category_id) == $cat->id)
  ...
  <div class="mt-2">
    ...
    <select name="supplier_id" id="supplier_id" class="form-control">
      ...
      @foreach($dsSuppliers as $sup)
        @if(old('supplier_id', $updatingModel->supplier_id) == $sup->id)
